...
def build_prediction_row(games, player, opponent, date, is_home):
    """Build one prediction row without rebuilding the full historical table."""
    player_games = games[
        games["PLAYER_NAME"].str.casefold() == player.casefold()
    ...
    opponent = opponent.upper()
    game_date = pd.Timestamp(date)
    days_rest = (game_date - latest_game["GAME_DATE"]).days
    ...
    previous_matchups = player_games[player_games["OPPONENT"] == opponent]
    row["H2H_GAMES_BEFORE"] = len(previous_matchups)
    for stat in TARGETS:
        row[f"{stat}_H2H_5"] = previous_matchups[stat].tail(5).mean()
    ...
    games_against_opponent = team_games[team_games["OPPONENT"] == opponent]
    for stat in TARGETS:
        row[f"OPP_ALLOWED_{stat}_10"] = games_against_opponent[stat].tail(10).mean()
